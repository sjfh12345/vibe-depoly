...
- **현재 구현:**
  ```typescript
  const loginResponsePromise = page.waitForResponse(..., { timeout: 2000 });
  const userResponsePromise = page.waitForResponse(..., { timeout: 2000 });
...
- **현재 구현:**
  ...
  await expect(page.locator('p:has-text("이메일 또는 비밀번호가 올바르지 않습니다.")')).toBeVisible({ timeout: 500 });
...
- **참고:** auth-signup 테스트도 5000ms를 사용하고 있어 일관성은 유지됨
- **권장 개선:** timeout을 500ms로 변경하거나 제거 (기본값 사용)

### ✅ /auth/login 페이지 완전 로드 후 테스트
- **요구사항:** /auth/login 페이지가 완전히 로드된 후 테스트할 것
- **구현 방식:**
  ```typescript
...
- **요구사항:** 실제 데이터를 사용할 것
- **구현 방식:**
  - 성공 시나리오: 실제 API 사용 (email: "a@aaa.com", password: "1234")
...
- **구현:**
  ...
  ```
- **결과:** 요구사항에 맞는 이메일과 비밀번호 사용 ✓

#### ✅ loginUser API: accessToken 정상 반환 확인
- **요구사항:** loginUser API: accessToken이 정상적으로 반환되는지 확인할 것
- **구현:**
  ```typescript
  const loginResponse = await loginResponsePromise;
  ...
  ```
- **결과:** accessToken 반환 확인 ✓

#### ✅ fetchUserLoggedIn API: _id, name 정상 반환 확인
- **요구사항:** fetchUserLoggedIn API: _id, name이 정상적으로 반환되는지 확인할 것
- **구현:**
  ```typescript
...
- **구현:**
  ...
  ```
- **결과:** data-testid 사용 ✓
- **참고:** Modal의 경우 auth-signup과 동일하게 텍스트 기반 선택자 사용 (기존 패턴 유지)

---

## 테스트 케이스 검토

### 테스트 케이스 1: 모든 입력값이 유효하면 로그인 버튼이 활성화됨
- ✅ /auth/login 페이지로 이동 (경로만 사용)
...
### 테스트 케이스 2: 로그인 성공 시 성공 모달 표시 및 일기목록 페이지 이동
- ✅ /auth/login 페이지로 이동 (경로만 사용)
- ✅ data-testid로 페이지 로드 대기
- ✅ 요구사항에 맞는 이메일("a@aaa.com")과 비밀번호("1234") 사용
- ✅ 실제 API 호출 (모킹 없음)
- ✅ waitForResponse로 loginUser API 응답 대기 (timeout 2000ms)
- ✅ accessToken 반환 확인
- ✅ waitForResponse로 fetchUserLoggedIn API 응답 대기 (timeout 2000ms)
- ✅ _id와 name 반환 확인
...
### 테스트 케이스 3: 로그인 실패 시 실패 모달 표시 및 닫힘
- ✅ API 모킹 사용 (실패 시나리오)
- ✅ /auth/login 페이지로 이동 (경로만 사용)
- ✅ data-testid로 페이지 로드 대기
- ✅ data-testid로 입력 필드 선택
...
### ⚠️ 페이지 로드 timeout 조정
- **현재:** `await page.waitForSelector('[data-testid="auth-login-section"]', { timeout: 5000 });`
- **요구사항:** network 통신이 아닌 경우 500ms 미만
- **권장 개선:**
  ```typescript
  await page.waitForSelector('[data-testid="auth-login-section"]', { timeout: 500 });
  ```
  또는 timeout 제거 (기본값 사용)
- **참고:** auth-signup 테스트도 5000ms를 사용하고 있어 일관성은 유지되지만, 요구사항에 더 부합하도록 500ms로 변경 권장
...
- [x] 이메일 "a@aaa.com" 사용
- [x] 비밀번호 "1234" 사용
- [x] loginUser API: accessToken 정상 반환 확인
- [x] fetchUserLoggedIn API: _id, name 정상 반환 확인
- [x] 로컬스토리지에 accessToken 저장 확인
- [x] 로컬스토리지에 user 정보 저장 확인
- [x] API 모킹 사용 (실패 시나리오)
- [x] page.goto 경로만 사용
